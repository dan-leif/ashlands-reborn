...
- The historical blocky transitions + yellow seam of the original override survive only in `TerrainTransitionStyle = Legacy`, kept as a revert option; the default MudBlend style replaces them with an organic fade. `LegacySmooth` keeps Legacy's binary look but not its blocky contours; its thin yellow fringe (one boundary-triangle wide) is inherent to any binary green/ash stamp.
