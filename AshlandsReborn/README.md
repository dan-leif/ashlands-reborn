...
- The historical blocky transitions + yellow seam of the original override survive only in `TerrainTransitionStyle = Legacy`, kept as a revert option; the styled paths replace them with an organic fade. The default `LegacySmooth` keeps Legacy's direct green-to-ash look but with MudBlend's curved contours; the yellow fringe is gone entirely (re-textured under a matched grass/ash mix).
